...
client_connection = QdrantClient(qdrant_host, api_key=qdrant_api_key, timeout=50)

qdrant_vector_config = qdrant_client.http.models.VectorParams(
    size=768, distance=qdrant_client.http.models.Distance.COSINE
)
...
payloads = df_anime[
    ["MAL_ID", "Name", "Japanese name", "Genres", "Score", "poster_url"]
].to_dict("records")

client_connection.upsert(
    collection_name=qdrant_collection_name,
    points=models.Batch(
        vectors=df_anime["vector_embeddings"], payloads=payloads, ids=df_anime.index
    ),
)
