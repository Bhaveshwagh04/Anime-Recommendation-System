...
def generate_recommendations(df_user, user_id, df_anime_ratings, k):
    client = qdrant_client.QdrantClient(qdrant_host, api_key=qdrant_api_key, timeout=50)

    # df_users_with_ratings
    filtered_user = df_user[df_user["user_id"] == user_id]

    anime_id_high_rating = filtered_user[filtered_user["rating"] > 5][
        "anime_id"
    ].tolist()
    anime_id_low_rating = filtered_user[filtered_user["rating"] < 5][
        "anime_id"
    ].tolist()

    high_rating = df_anime_ratings[
        df_anime_ratings["MAL_ID"].isin(anime_id_high_rating)
    ].index
    low_rating = df_anime_ratings[
        df_anime_ratings["MAL_ID"].isin(anime_id_low_rating)
    ].index
    ...
    sorted_scored_points = sorted(
        k_recommendations, key=lambda x: x.score, reverse=True
    )
