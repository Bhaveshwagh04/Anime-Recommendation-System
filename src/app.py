...
def main():
    ...
    if my_expander.button("Recommend"):
        ...
        anime_info = generate_recommendations(
            df_user=df_users_with_rating,
            user_id=selected_user_id,
            df_anime_ratings=df_anime,
            k=5,
        )
        ...
        for anime, col in zip(anime_info, cols):
            ...
            print(f"col: {col}")
            col.write(
                f'<b style="color:#E50914">English Name</b>:<b> {anime["name"]}</b>',
                unsafe_allow_html=True,
            )
            col.write(
                f'<b style="color:#E50914">Japanese Name</b>:<b> {anime["japanese_name"]}</b>',
                unsafe_allow_html=True,
            )
            st.write("##")
            ...
            col.write(
                f'<b style="color:#DB4437">Rating</b>:<b> {anime["score"]}</b>',
                unsafe_allow_html=True,
            )
            col.write(
                f'<b style="color:#DB4437">Genres</b>: <b> {anime["genres"]} <b> ',
                unsafe_allow_html=True,
            )
